...
void LLMRequestTask::run()
{
    ...
    if (!m_config.context.isEmpty()) {
        requestJson["context"] = m_config.context;
    }
            ...
            if (reply->error() == QNetworkReply::NoError) {
                ...
                if (parseError.error == QJsonParseError::NoError) {
                    ...
                    for (const auto &modelValue : models) {
                        ...
                        info.size = static_cast<qint64>(modelObj["size"].toDouble());
                        info.digest = modelObj["digest"].toString();
                        info.modifiedAt = QDateTime::fromString(modelObj["modified_at"].toString(), Qt::ISODate);
...
void PromptTemplateManager::loadTemplates(const QString &templateDir)
{
    ...
    if (!templateDirectory.exists()) {
        ...
        gameTemplate.template = "你是一個來自{{academy}}學院的{{character_type}}。你的性格是{{personality}}。請以這個角色的身份回應玩家的話：{{user_message}}";
        ...
        gameTemplate.tags = QStringList{"遊戲", "角色", "對話"};
        ...
        skillTemplate.template = "生成一個{{skill_type}}技能的描述：\n技能名稱：{{skill_name}}\n效果等級：{{level}}\n學院：{{academy}}\n請包含技能效果、冷卻時間和消耗。";
        ...
        skillTemplate.tags = QStringList{"遊戲", "技能", "描述"};
    ...
    for (const QString &fileName : templateFiles) {
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            if (error.error == QJsonParseError::NoError) {
                ...
                tmpl.template = obj["template"].toString();
...
QString PromptTemplateManager::renderTemplate(const QString &templateId, const QMap<QString, QString> &variables) const
{
    ...
    QString result = m_templates[templateId].template;
