 ...
 * @brief LLM請求任務 (用於線程池)
 */
class LLMRequestTask : public QRunnable
{
public:
    LLMRequestTask(const LLMRequestConfig &config, 
                   const OllamaServerConfig &server);
    
    void run() override;
    
    // 回調函數設置
    void setCallback(std::function<void(const LLMResponse&)> callback);

private:
    LLMRequestConfig m_config;
    OllamaServerConfig m_server;
    std::unique_ptr<QNetworkAccessManager> m_networkManager;
    std::function<void(const LLMResponse&)> m_callback;
